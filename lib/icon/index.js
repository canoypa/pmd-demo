...
class icon extends component {
  constructor(icname, opt = { width: null, height: null, id: null, classList: null }) {
    ...
    if (!paths) {
      throw new Error(`${icname} is invalid value`);
    }

    const svgNode = document.createElementNS('http://www.w3.org/2000/svg', 'svg');
    svgNode.setAttribute('viewBox', '0 0 24 24');
    svgNode.classList.add('pmd-icon');
    super(svgNode);

    if (opt.width) {
      svgNode.setAttribute('width', opt.width);
    }
    if (opt.height) {
      svgNode.setAttribute('height', opt.height);
    }
    if (opt.id) {
      svgNode.id = opt.id;
    }
    if (opt.classList) {
      if (typeof opt.classList === 'string') {
        svgNode.classList.add(opt.classList);
      } else if (typeof opt.classList === 'object') {
        for (let className of opt.classList) {
